...
- **Presentation:** passes as a young boy in-world (deliberate on her part, not incidental). **Whether Marian actually knows her secret is currently ambiguous/undecided** — what's been shown on-page is Marian developing her own independent suspicion about Pip (Ch7, Ch12, Ch14, Ch15), not a confirmed disclosure. Ch15 ends with Pip asking to speak with Marian privately; that conversation is the natural place to resolve whether Marian already knew, is about to find out, or something in between. **Do not write a scene assuming Marian has secret prior knowledge of Pip's gender until that conversation actually happens and settles it.**
...
- **Continuity notes:** the exact nature of the Marian/Pip private axis is intentionally left open until the pending Ch15-cliffhanger conversation resolves it (see above).
...
- **LOCKED NARRATION RULE:** Pip's gender stays hidden from the **reader**, not just other characters, until her actual reveal (same timing as the wizard reveal — one unified reveal, not staggered). This means **narration uses "he/him/his" for Pip consistently**, not just in-world dialogue. Do not use "they/their" as a hedge — commit fully to "he" until the reveal chapter, exactly as Ch6 and (corrected) Ch7 already do. This applies to every future chapter Pip appears in.

---

## The Animal Cast

Three animals belonging to (or adopted by) the gang, each granted telepathic ability by Master Thistlewood — this is how he keeps informed of the gang's doings from a distance without appearing himself. **This mechanism is a locked continuity fact, but reader-facing exposure of it is deliberately staggered** (see `story-bible.md` for the exact rule): the animals' personalities and banter with each other can be shown on the page now; the fact that they report to Thistlewood specifically stays hidden from the reader until the proper wizard-reveal story, same as everything else about him.
