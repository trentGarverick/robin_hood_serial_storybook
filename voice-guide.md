...
**This principle isn't limited to the Pip/Thistlewood pair — it applies to any character whose blurb would spoil a plot outcome.** Example (Ch39): Witchfinder Crane's investigation ends in confusion and a "softened" memory after a major confrontation, but his `guide.json` blurb was correctly left untouched — it describes his method/personality ("investigating... doesn't seem especially interested in whether the stories are true") rather than his outcome, so it stays safely evergreen without needing to lie or go stale. When updating any character's guide entry after a big story development, ask: does this blurb reveal what *happens* to them, or just who they *are*? Only the latter is safe to state; the former should stay exactly as it was before the event, even if that means the entry undersells how much has actually changed for that character in the story itself.

**Pip's pronouns:** **RULE CHANGED as of Ch16 (Story 4) — the reveal has happened.** Narration now uses "she/her/hers" for Pip consistently in every scene, regardless of who's present — the reader permanently knows now. **UPDATE (Ch39): Robin is now also informed** (previously only Marian) — in his own dialogue going forward, Robin may correctly use "she/her" for Pip. Uninformed characters (everyone except Marian and Robin) still say "he" **in their own dialogue**, since their knowledge hasn't changed — this currently means Will, Long John, and Brother Crumb. Don't hedge with "they" in either direction. Full rule and reasoning in `characters.md`. (For any pre-Ch16 chapter references or excerpts, the old "he" throughout applies — this change is forward-only from Ch16; the Robin-specific update is forward-only from Ch39.)
